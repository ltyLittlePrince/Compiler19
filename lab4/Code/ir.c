        ...
        if((interCodesList[i]->kind == 4 || interCodesList[i]->kind == 5) && \
          ...
          interCodesList[i+1]->u.kind_4567.op2->kind == INT_CONST
            ...
            if((interCodesList[i]->kind == 4 && interCodesList[i+1]->kind == 4)){
                if(a + b < -2147483648 && a + b > 2147483647) continue;
...
#endif
        // printInterCode(interCodesList[i]); //exp04,不输出中间代码了。
    }
    return numDelete;
}
...
Bool eqOperand(struct Operand* op1, struct Operand* op2){
    int res = 0;
    switch (op1->kind){
        case VARIABLE: case FUNCTION: case GET_VAR_VALUE: case GET_VAR_ADDRESS:      
            if(op2->kind == VARIABLE || op2->kind == FUNCTION || \
              op2->kind == GET_VAR_VALUE || op2->kind == GET_VAR_ADDRESS)
                res = (strcmp(op1->u.va_name, op2->u.va_name) == 0);   break;
        case FLOAT_CONST:       res =  (op1->u.float_const == op2->u.float_const);      break;
        case INT_CONST:         res =  (op1->u.int_const == op2->u.int_const);          break;
        case TEMP: case GET_TEMP_VALUE: case GET_TEMP_ADDRESS:
            if(op2->kind == TEMP || op2->kind == GET_TEMP_VALUE || op2->kind == GET_TEMP_ADDRESS)      
                res = (op1->u.tmp->no == op2->u.tmp->no); break;
...
void printInterCode(struct InterCode* ic) {
    switch (ic->kind) {
        ...
        case 2: if (first_def == 1) { first_def = 0; } else { } // First function declaration doesn't need an interval blank.
